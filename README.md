...
processes one current event as the invocation's sole user message; extensions add formatted,
measured context sections to the system message.
...
and each compiled context. Its Live Memory Graph (`http://localhost:8787/trace?view=graph`) uses pinned `vgpu` 0.3.1 and WebGPU to render a rotating, depth-aware cloud of durable events, causation, and semantic links. It opens on all live memories; optional collapsed filters accept exact event type, source, and correlation values. Correlation is a halo/grouping, never a fabricated edge. Selecting a memory separately highlights its visible causal ancestors and descendants. Event families are color-coded. The graph defaults to the newest 250 events, caps nodes at 1,000 and semantic links at 2,000, and marks causal parents outside
...
camera rotation or zoom; hover reveals immediate neighbors, and double-clicking a node opens its causal trace. New memories form with a bounded six-second GPU animation. WebGPU is mandatory: use a WebGPU-capable browser on `localhost`; unavailable or lost devices produce a visible hard error, not another renderer. Rebuild the checked-in bundle with `npm ci --ignore-scripts && npm run build:memory-graph`. See [`docs/third-party.md`](docs/third-party.md). A
